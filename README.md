...
> In this template, we use `@homefront` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@homefront` to something like `@my-company` or `@project-name`.
...
- For the auth proxy server, set `AUTH_REDIRECT_PROXY_URL` to the same as above. Lastly, set `AUTH_SECRET` **to the same value as in the Next.js app** for preview environments.
